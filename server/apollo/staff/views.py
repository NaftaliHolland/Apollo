...
def get_teacher_count(request):
    ...
    try:
        ...
        return Response({"message": "School with the given id does not exist"}, status=status.HTTP_400_BAD_REQUEST)
    teacher_count = Teacher.objects.filter(user__school=school).count()
